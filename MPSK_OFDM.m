...
for n=1:10
    ...
    for k=1:length(theta_0)
        ...
        theta=(theta_0(k)./(d0+v*t+0.5*a*t.^2).^3.34).';
...
%         
%         EsN0_dB(k)=10*log10(EsN0(k));
%         EbN0(k)=EsN0(k)/nbitpsk;
%         EbN0_dB(k)=10*log10(EbN0(k));
%         
%         % calculate theta estimation accuracy
%         mse_theta(n,k)=immse(theta,mean(est_theta));
        ...
        Btheta=(theta_0(k)./(d0+v*Bt+0.5*a*Bt.^2).^3.34).';
...
%         BEsN0_dB(k)=10*log10(BEsN0(k));
%         BEbN0(k)=BEsN0(k)/nbitpsk;
%         BEbN0_dB(k)=10*log10(BEbN0(k));
        ...
        Ctheta=(theta_0(k)./(d0+v*Ct+0.5*a*Ct.^2).^3.34).';
        ...
        % parallalize ch data
        Cpardata=reshape(Cch_rem_pilot, 2,[]).';

        % separate data and training matrix
        Crecpilot=abs(Cpardata(:,1));
        Crecdata=Cpardata(:,2);
        ...
        % estimate psk_data_tx
        Cresp=reshape(interp(Crecpilot,2),2,[]).';
        Cpsk_data_rx=Crecdata./Cresp(:,2);

        % serialize psk data
        Cserdata=reshape(Cpsk_data_rx.', subcar*nsym/2,[]).';
...
%         CEsN0_dB(k)=10*log10(CEsN0(k));
%         CEbN0(k)=CEsN0(k)/nbitpsk;
%         CEbN0_dB(k)=10*log10(CEbN0(k));
        ...
        BBtheta=(theta_0(k)./(d0+v*BBt+0.5*a*BBt.^2).^3.34).';
...
%         BBEsN0_dB(k)=10*log10(BBEsN0(k));
%         BBEbN0(k)=BBEsN0(k)/nbitpsk;
%         BBEbN0_dB(k)=10*log10(BBEbN0(k));
        ...
        %%
        % Channel Effect 1    
        CCt=(0:(nbitsym*nsym*1.5-1))*dt;
        CCtheta=(theta_0(k)./(d0+v*CCt+0.5*a*CCt.^2).^3.34).';
        ...
        % parallalize ch data
        CCpardata=reshape(CCch_rem_pilot, 3,[]).';

        % separate data and training matrix
        CCrecdata=CCpardata(:,2:3);
        CCrecpilot=abs(CCpardata(:,1));
        
        % calculate SNR
        CCEsN0(n,k)=mean(abs(CCch_rem_pilot(symbols)).^2)/mean(abs(noise3).^2);

        % estimate psk_data_tx
        CCresp=reshape(interp(CCrecpilot,3),3,[]).';
        CCpsk_data_rx=CCrecdata./CCresp(:,2:3);

        % serialize psk data
        CCserdata=reshape(CCpsk_data_rx.', subcar*nsym/2,[]).';
...
%         CCEsN0_dB(k)=10*log10(CCEsN0(k));
%         CCEbN0(k)=CCEsN0(k)/nbitpsk;
%         CCEbN0_dB(k)=10*log10(CCEbN0(k));
        ...
        BBBtheta=(theta_0(k)./(d0+v*BBBt+0.5*a*BBBt.^2).^3.34).';
...
%         BBBEsN0_dB(k)=10*log10(BBBEsN0(k));
%         BBBEbN0(k)=BBBEsN0(k)/nbitpsk;
%         BBBEbN0_dB(k)=10*log10(BBBEbN0(k));
...
%% THIS IS FOR COMB TYPE CE ALGORITHM (3 data symbol, 1 pilot symbol)
        ...
        CCCtheta=(theta_0(k)./(d0+v*CCCt+0.5*a*CCCt.^2).^3.34).';
        ...
        % parallalize ch data
        CCCpardata=reshape(CCCch_rem_pilot, 4,[]).';

        % separate data and training matrix
        CCCrecdata=CCCpardata(:,2:4);
        CCCrecpilot=abs(CCCpardata(:,1));
        
        % calculate SNR
        CCCEsN0(n,k)=mean(abs(CCCch_rem_pilot(symbols)).^2)/mean(abs(noise4).^2);

        % estimate psk_data_tx
        CCCresp=reshape(interp(CCCrecpilot,4),4,[]).';
        CCCpsk_data_rx=CCCrecdata./CCCresp(:,2:4);

        % serialize psk data
        CCCserdata=reshape(CCCpsk_data_rx.', subcar*nsym/2,[]).';
...
figure
semilogy(BEbN0_dB,BBER)
hold on
semilogy(BBEbN0_dB,BBBER) 
hold on
semilogy(BBBEbN0_dB,BBBBER) 
hold on
semilogy(CEbN0_dB,CBER)
hold on
semilogy(CCEbN0_dB,CCBER)
hold on
