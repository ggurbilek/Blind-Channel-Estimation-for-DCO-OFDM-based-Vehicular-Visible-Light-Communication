...
d0=1;
%theta_0=0.3513;
%theta_0_dB = [-3.54283773192138;-17.1914927779135;-24.4824294899401;-26.8132568409099;-30.3584071409069;-33.5493022466249;-36.1852237045423;-39.8314632801301;-40.7789752000158;-42.7140802121108].'+40; % 40dB PD gain // outdoor partial cloudy
theta_0_dB = [10.6585269745239;1.23214524237809;-5.38336102081100;-10.0264280978343;-13.4981962315402;-16.0637998034093;-18.0130152156046;-20.0843836733988;-21.8108362296485;-23.4081235324811].'+40; % 40dB PD gain // parking lot
...
figure
semilogy(distance,const*1/2*(1-BBER),'pb-')
hold on
semilogy(distance,const*2/3*(1-BBBER),'sr-') 
hold on
semilogy(distance,const*3/4*(1-BBBBER),'dy-')
hold on
semilogy(distance,const*1/2*(1-CBER),'+k-')
hold on
semilogy(distance,const*2/3*(1-CCBER),'xc-')
hold on
semilogy(distance,const*3/4*(1-CCCBER),'*m-')
hold on
semilogy(distance,const*(1-SBER),'ob-')
hold on
semilogy(distance,const*(1-BER),'^g-')
legend('Block PS1','Block PS2','Block PS3','Comb PS1','Comb PS2','Comb PS3','ST','Proposed')
xlabel('Distance (m)')
ylabel('Spectral Efficiency (C [bits/s/Hz])')
%title('parking lot, 0deg')
grid on
