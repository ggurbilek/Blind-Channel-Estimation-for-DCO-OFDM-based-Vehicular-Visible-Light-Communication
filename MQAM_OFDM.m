...
for f=2:6
    ...
    for g=1:7
        ...
        qamsnrber{f,g}={BER,BBER,BBBER,BBBBER,CBER,CCBER,CCCBER,SBER;EbN0_dB,BEbN0_dB,BBEbN0_dB,BBBEbN0_dB,CEbN0_dB,CCEbN0_dB,CCCEbN0_dB,SEbN0_dB};
    end
...
%         'Comb type CE (1 data - 1 pilot symbol)','Comb type CE (2 data - 1 pilot symbol)',...
%         'Comb type CE (3 data - 1 pilot symbol)',['ST-based CE, M=' num2str(SM) ' \alpha=' num2str(Salpha)],'Proposed CE')
%     xlabel('Eb/N0 (dB)')
%     ylabel('BER')
%     title([num2str(qam) ' QAM CE BER'])
% 
%     const = (nbitqam*subcar/2)/(nfft+ncp);
% 
%     figure
%     semilogy(BEbN0_dB,const*1/2*(1-BBER),'pb-')
%     hold on
%     semilogy(BBEbN0_dB,const*2/3*(1-BBBER),'sr-') 
%     hold on
%     semilogy(BBBEbN0_dB,const*3/4*(1-BBBBER),'dy-')
%     hold on
%     semilogy(CEbN0_dB,const*1/2*(1-CBER),'+k-')
%     hold on
%     semilogy(CCEbN0_dB,const*2/3*(1-CCBER),'xc-')
%     hold on
%     semilogy(CCCEbN0_dB,const*3/4*(1-CCCBER),'*m-')
%     hold on
%     semilogy(SEbN0_dB,const*(1-SBER),'ob-')
%     hold on
%     semilogy(EbN0_dB,const*(1-BER),'^g-')
%     legend('Block PS1','Block PS2','Block PS3','Comb PS1','Comb PS2','Comb PS3',['ST M=' num2str(SM) ' \alpha=' num2str(Salpha)],'Proposed')
%     xlabel('Eb/N0 (dB)')
%     ylabel('Spectral Efficiency (C [bits/s/Hz])')
%     title([num2str(qam) ' QAM CE TPUT'])
    
end

save('qamsnrber.mat','qamsnrber');