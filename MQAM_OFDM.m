...
for n=1:10
    ...
    for k=1:length(theta_0)
        ...
        Cqam_data_rx=Crecdata./Cresp(:,2);

        % serialize qam data
        Cserdata=reshape(Cqam_data_rx.', subcar*nsym/2,[]).';
...
%% THIS IS FOR COMB TYPE CE ALGORITHM (2 data symbol, 1 pilot symbol)
        ...
        CCqam_data_rx=CCrecdata./CCresp(:,2:3);

        % serialize qam data
        CCserdata=reshape(CCqam_data_rx.', subcar*nsym/2,[]).';
...
%% THIS IS FOR COMB TYPE CE ALGORITHM (3 data symbol, 1 pilot symbol)
        ...
        CCCqam_data_rx=CCCrecdata./CCCresp(:,2:4);

        % serialize qam data
        CCCserdata=reshape(CCCqam_data_rx.', subcar*nsym/2,[]).';
...
figure
semilogy(BEbN0_dB,nbitqam*1/2*(1-BBER),'pb-')
hold on
semilogy(BBEbN0_dB,nbitqam*2/3*(1-BBBER),'sr-') 
hold on
semilogy(BBBEbN0_dB,nbitqam*3/4*(1-BBBBER),'dy-')
hold on
semilogy(CEbN0_dB,nbitqam*1/2*(1-CBER),'+k-')
hold on
semilogy(CCEbN0_dB,nbitqam*2/3*(1-CCBER),'xc-')
hold on
semilogy(CCCEbN0_dB,nbitqam*3/4*(1-CCCBER),'*m-')
hold on
semilogy(EbN0_dB,nbitqam*(1-BER),'^g-')
